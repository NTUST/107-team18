...
from courses.forms import CourseFilesForm
...
def courses(request):
    if request.method == "POST":  # Search
        ...
        if mode == "admin":
            courses = (
                CourseInformation.objects.filter(
                    course_administrator__admin_name__icontains=codintion
                )
                .annotate(file_count=Count("coursefile"))
                .order_by("-file_count")
            )
        elif mode == "teacher":
            courses = (
                CourseInformation.objects.filter(course_teacher__icontains=codintion)
                .annotate(file_count=Count("coursefile"))
                .order_by("-file_count")
            )
        elif mode == "name":
            courses = (
                CourseInformation.objects.filter(course_name__icontains=codintion)
                .annotate(file_count=Count("coursefile"))
                .order_by("-file_count")
            )
        else:
...
def courses_detail(request, id):
    ...
    handouts = CourseFile.objects.filter(
        course=course, cfile_class__icontains="handout"
    )
    homeworks = CourseFile.objects.filter(
        course=course, cfile_class__icontains="homework"
    )
